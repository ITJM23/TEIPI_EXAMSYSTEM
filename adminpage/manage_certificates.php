...
@sqlsrv_query($con3, "IF COL_LENGTH('dbo.Certificates','Expiration_Date') IS NULL ALTER TABLE dbo.Certificates ADD Expiration_Date DATETIME NULL");
...
@sqlsrv_query($con3, "IF COL_LENGTH('dbo.Certificates','is_Unlimited') IS NULL ALTER TABLE dbo.Certificates ADD is_Unlimited BIT DEFAULT 0");

// Handle updates
$update_msg = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['certificate_id'])) {
    $cid = (int)($_POST['certificate_id']);
    $isUnlimited = isset($_POST['is_unlimited']) ? 1 : 0;
    $expType = $_POST['exp_type'] ?? 'none'; // none | days | date
    $expValue = trim($_POST['exp_value'] ?? '');

    if ($isUnlimited) {
        $sql = "UPDATE dbo.Certificates SET is_Unlimited = 1, Expiration_Date = NULL WHERE Certificate_ID = ?";
        $ok = sqlsrv_query($con3, $sql, [$cid]);
    } else {
        if ($expType === 'days' && $expValue !== '' && is_numeric($expValue)) {
          $sql = "UPDATE dbo.Certificates SET is_Unlimited = 0, Expiration_Days = ?, Expiration_Date = NULL WHERE Certificate_ID = ?";
          $ok = sqlsrv_query($con3, $sql, [intval($expValue), $cid]);
          ...
          if ($dateInput === false || $dateInput === '' || $dateInput === '1970-01-01') {
            ...
            $sql = "UPDATE dbo.Certificates SET is_Unlimited = 0, Expiration_Date = ?, Expiration_Days = NULL WHERE Certificate_ID = ?";
            $ok = sqlsrv_query($con3, $sql, [$dateInput, $cid]);
          }
        } else {
          $sql = "UPDATE dbo.Certificates SET is_Unlimited = 0, Expiration_Date = NULL, Expiration_Days = NULL WHERE Certificate_ID = ?";
            $ok = sqlsrv_query($con3, $sql, [$cid]);
        }
    }
    if ($update_msg === '') {
      $update_msg = $ok ? 'Certificate updated.' : 'Update failed.';
    }
}

// Fetch certificates
$rows = [];
$stmt = sqlsrv_query($con3, "SELECT Certificate_ID, Certificate_Name, Certificate_Description, Expiration_Date, Expiration_Days, is_Unlimited FROM dbo.Certificates ORDER BY Certificate_Name");
if ($stmt) {
...
<?php include 'admin_navbar.php'; ?>
  <div class="max-w-6xl mx-auto p-6">
    <h1 class="text-2xl font-bold mb-4">Manage Certificates Expiration</h1>
    <div class="mb-4">
      <a href="recalculate_expirations.php" class="inline-flex items-center px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded" title="Recompute employee certificate expirations based on current policies">Recalculate Expirations</a>
    </div>
    <?php if (!empty($update_msg)): ?>
      <div class="mb-4 p-3 rounded <?php echo ($update_msg==='Certificate updated.')?'bg-emerald-50 text-emerald-700':'bg-red-50 text-red-700'; ?>"><?php echo htmlspecialchars($update_msg); ?></div>
    <?php endif; ?>
      ...
      <?php foreach ($rows as $row): ?>
        <div class="bg-white shadow rounded p-4">
          <div class="flex justify-between items-center mb-2">
            <div>
              <div class="text-lg font-semibold"><?php echo htmlspecialchars($row['Certificate_Name']); ?></div>
              <div class="text-sm text-slate-500"><?php echo htmlspecialchars($row['Certificate_Description'] ?? ''); ?></div>
            </div>
          ...
          <form method="POST" class="space-y-2">
            <input type="hidden" name="certificate_id" value="<?php echo (int)$row['Certificate_ID']; ?>" />
